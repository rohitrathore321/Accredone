...
class AppPermission {
    ...
    requestAllPermissions = async () => {
        const iosPerm = [ PERMISSIONS.IOS.CAMERA, PERMISSIONS.IOS.PHOTO_LIBRARY, PERMISSIONS.IOS.MEDIA_LIBRARY, PERMISSIONS.IOS.CALENDARS ];
        const androidPerm = [PERMISSIONS.ANDROID.CAMERA, PERMISSIONS.ANDROID.READ_MEDIA_IMAGES, PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE, PERMISSIONS.ANDROID.WRITE_CALENDAR];
        const response = await requestMultiple(Platform.OS == 'android' ? androidPerm : iosPerm).then(result => {
            return result
        });
        return response;
    }
    ...
    checkAllPermissions = async () => {
        const iosPerm = [ PERMISSIONS.IOS.CAMERA, PERMISSIONS.IOS.PHOTO_LIBRARY, PERMISSIONS.IOS.MEDIA_LIBRARY, PERMISSIONS.IOS.CALENDARS ];
        const androidPerm = [PERMISSIONS.ANDROID.CAMERA, PERMISSIONS.ANDROID.READ_MEDIA_IMAGES, PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE, PERMISSIONS.ANDROID.WRITE_CALENDAR];
        const response = await checkMultiple(Platform.OS == 'android' ? androidPerm : iosPerm).then(result => {
            if(Platform.OS === 'android'){
                return result[PERMISSIONS.ANDROID.CAMERA] === 'granted'
            }else {
                return result[PERMISSIONS.IOS.CAMERA] === 'granted'
